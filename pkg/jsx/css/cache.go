...
import (
	...
	"github.com/morlay/genshin-level-up/pkg/jsx/hash"
	"github.com/morlay/genshin-level-up/pkg/jsx/postcss"
)
...
func (s *SerializedStyles) ToCSS(key string) []byte {
	...
	n := postcss.Parse(fmt.Sprintf(".%s-%s{%s}", key, s.Name, string(s.Styles)))

	n.FormatTo(b, &postcss.FormatOpt{
		OneLine: true,
	})
